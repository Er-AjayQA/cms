...
import { useSuperadminSubscription } from "@/features/superadmin/subscriptions/providers/superadmin-subscriptions-provider";
...
export const SuperadminSubscriptionPlanListing = () => {
  const {
    StatusToggleBadge,
    openCreateForm,
    openRecord,
      ...
      <Card className="overflow-hidden border-border/70 bg-white/70">
        <CardContent className="p-0">
          <Table>
            ...
            <TableBody>
              {isListLoading ? (
                <TableListingSkeleton listingLength={5} columnLength={6} />
              ) : records.length <= 0 ? (
                <TableListingNoRecords span={6} />
              ) : (
                records.map((plan, idx) => (
                  <TableRow key={idx} className="bg-white/20">
                    ...
                    <TableCell className="text-center py-1">
                      <StatusToggleBadge status={plan.status} id={plan.id} />
                    </TableCell>
                    <TableCell className="py-1">
                      <div className="flex justify-center gap-1">
                        <Button
                          size="icon"
                          variant="none"
                          onClick={() => openRecord(plan?.id, "view")}
                        ...
                        <Button
                          size="icon"
                          variant="none"
                          onClick={() => openRecord(plan?.id, "edit")}
                          disabled={isRecordLoading}
